...
The live tracker also reports predicted efficiency, current apps/hour, current AHT, and current efficiency for each planned department and the active segment. Department performance is isolated, so work recorded in AA/PCA does not alter BCA efficiency (or vice versa). Predicted efficiency compares the complete rounded plan with its raw queue-rate expectation, while current efficiency compares recorded completions with the unrounded target earned over relevant productive time elapsed so far.

**Menu → Day Details** shows the recalculated plan for the whole day, each department, each queue, and each productive segment. It includes expected whole apps, recalculated apps/hour, completed apps, productive time, predicted efficiency, current apps/hour, and current efficiency, and refreshes those time-sensitive values live while it is open. Current values use only productive time elapsed and completions recorded so far in that row's scope. Queue-based whole-day, department, and queue totals include recorded work even when that particular productive queue has no scheduled segment; segment rows remain limited to work attached to that exact segment. Predicted efficiency compares the rounded, optionally modified target with the raw queue-rate expectation; rounding normally places it above 100%, while a manual reduction can take it below 100%.
